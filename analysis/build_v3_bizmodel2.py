...
print(f"\nTotal recoverable GMV (cancellation-only lens, if SBB/DSE brands matched FBN's {FBN_CANCEL:.1f}% cancel rate): AED {lowtier['recoverable_gmv_if_fbn_cancel'].sum():.0f} (Aug'26, 16-day) -> monthly-equiv AED {lowtier['recoverable_gmv_if_fbn_cancel'].sum()/16*30.4:.0f}")
